...
|---------------------|---------------------------|------|------------------|
| API Gateway | Azure API Management | Premium | ~£2,500 |
...
| Identity | Microsoft Entra ID + External ID | P1 | ~£200 |
...
| Standard v2 | ~£250 | SLA, auto-scale | Small production |
| Premium | ~£2,500 | VNet, multi-region, self-hosted gateway | Enterprise production |

**Estimated Cost for This Project**:

| Resource | Configuration | Monthly Cost | Notes |
|----------|---------------|--------------|-------|
| APIM Premium | 1 unit, UK South | £2,500 | Required for VNet integration, Private Link |
| Additional unit (DR) | UK West standby | £2,500 | Active-passive DR |
...
| Pillar | Rating | Notes |
|--------|--------|-------|
| **Reliability** | 5/5 | 99.99% SLA (Premium), multi-region, availability zones |
| **Security** | 5/5 | OAuth 2.0, JWT validation, IP filtering, mutual TLS, WAF integration |
| **Cost Optimization** | 3/5 | Premium tier is expensive; Standard v2 may suffice for initial phases |
| **Operational Excellence** | 5/5 | Azure Monitor integration, built-in analytics, GitOps for API configurations |
| **Performance Efficiency** | 5/5 | Response caching, connection pooling, <10ms gateway overhead achievable |

**Azure Security Benchmark Alignment**:

| Control | Status | Implementation |
|---------|--------|----------------|
| NS-1: Network Security | Implemented | VNet integration, Private Link for backend services |
| IM-1: Identity Management | Implemented | OAuth 2.0, API keys, Entra ID integration |
| DP-1: Data Protection | Implemented | TLS 1.2+, encrypted backend connections |
| LT-1: Logging & Threat Detection | Implemented | Application Insights, Azure Monitor |

**UK Region Availability**:
- UK South: Available (all tiers)
- UK West: Available (all tiers)

---

#### Alternative: Azure Container Apps with Custom Gateway (Envoy/NGINX)

A custom API gateway built on Envoy or NGINX deployed to Container Apps would provide more control but significantly increase operational overhead. Not recommended for this project given the requirements for a built-in developer portal, analytics, and rate limiting — all provided out-of-the-box by APIM.

#### Comparison Matrix

| Criteria | API Management (Premium) | Custom Gateway on Container Apps |
|----------|--------------------------|----------------------------------|
...
| Circuit Breaker | Built-in | Must implement |
| Operational Overhead | Low (managed) | High |
| UK Availability | UK South, UK West | UK South, UK West |

**Recommendation**: Azure API Management (Premium) — provides the full API gateway feature set required by the requirements with minimal custom development. The built-in developer portal addresses BR-003 directly.
...
- **Dapr integration**: Sidecar-based distributed application runtime for pub/sub, state management, service invocation
- **KEDA scaling**: Event-driven autoscaling from Service Bus queues, HTTP traffic, custom metrics (addresses NFR-P-004)
...
| Adapter Service | 1 vCPU, 2GB RAM, 2-10 replicas | £300 | Auto-scales with traffic |
| Webhook Service | 0.5 vCPU, 1GB RAM, 1-5 replicas | £150 | Event-driven, scales to zero |
| Background Workers | 1 vCPU, 2GB RAM, 2 replicas | £150 | Analytics aggregation, health checks |
| **Total** | | **~£800** | Consumption plan |

**Azure Well-Architected Assessment**:

| Pillar | Rating | Notes |
|--------|--------|-------|
| **Reliability** | 4/5 | Availability zones, auto-restart, health probes |
| **Security** | 4/5 | Managed identity, VNet integration, secrets management |
| **Cost Optimization** | 5/5 | Pay-per-use, scale to zero for batch workloads |
| **Operational Excellence** | 5/5 | Managed platform, integrated logging, zero cluster management |
| **Performance Efficiency** | 4/5 | KEDA-based autoscaling, rapid scale-out |
...
**Key Features**:
- **PostgreSQL compatibility**: Full PostgreSQL engine with extensions (JSONB for metadata, full-text search)
- **Zone-redundant HA**: Automatic failover across availability zones in UK South (addresses NFR-A-001)
...
| Resource | Configuration | Monthly Cost | Notes |
|----------|---------------|--------------|-------|
| Primary Instance | General Purpose, 4 vCores, 128GB storage | £400 | Zone-redundant HA |
...
| Pillar | Rating | Notes |
|--------|--------|-------|
| **Reliability** | 5/5 | Zone-redundant HA, geo-redundant backup, 99.99% SLA |
| **Security** | 5/5 | Encryption at rest/transit, VNet integration, Entra ID auth |
| **Cost Optimization** | 4/5 | Right-sized tiers, reserved capacity available |
| **Operational Excellence** | 5/5 | Managed patching, monitoring, automated backups |
| **Performance Efficiency** | 4/5 | Read replicas, connection pooling via PgBouncer |

**UK Region Availability**: UK South (Zone-Redundant HA available), UK West (Same-Zone HA available)
...
**Key Features**:
- **External ID**: Self-service developer registration with email/social sign-up (addresses FR-003)
- **MFA**: Built-in multi-factor authentication for all user tiers (addresses NFR-SEC-001)
...
- **Conditional Access**: Location-based and risk-based access policies
...
| Resource | Configuration | Monthly Cost | Notes |
|----------|---------------|--------------|-------|
| Entra ID P1 | Per-user licensing for admins | £150 | MFA, Conditional Access |
| External ID | Per-authentication pricing | £50 | Developer self-service sign-up |
| **Total** | | **~£200** | |
...
**Key Features**:
- **Global load balancing**: Anycast routing to nearest edge PoP (192 PoPs globally)
- **WAF managed rules**: OWASP Top 10 protection, bot management, custom rules (addresses NFR-SEC-007)
- **DDoS protection**: Layer 3/4/7 DDoS protection built-in
...
| WAF Policy | Managed + custom rules | £100 | OWASP protection |
| **Total** | | **~£500** | |
...
- Stores upstream API credentials with per-department isolation (separate keys per department)
...
The microservices layer consists of independently deployable adapters for each upstream government API (HMRC, Companies House, DVLA, NHS Digital, Environment Agency, Ordnance Survey, Land Registry, DWP), plus shared services for catalogue management, developer account management, and analytics aggregation.

Azure Front Door sits in front of APIM providing CDN caching for the developer portal, WAF protection, and DDoS mitigation. All data resides in UK South with geo-redundant backups to UK West.
...
graph TB
    ...
    subgraph "Azure Front Door Premium"
        FD[Azure Front Door<br/>CDN + WAF + DDoS]
    end

    subgraph "Azure UK South - Primary Region"
        subgraph "Identity Layer"
            EntraID[Microsoft Entra ID<br/>+ External ID]
        end

        subgraph "API Gateway Layer"
            APIM[Azure API Management<br/>Premium]
        ...
        subgraph "Data Layer"
            PgSQL[(Azure PostgreSQL<br/>Flexible Server)]
...
|-----------|---------------|---------|------|
| Edge Security | Azure Front Door | CDN, WAF, DDoS protection | Premium |
| API Gateway | API Management | Request routing, auth, rate limiting | Premium |
| Developer Portal | API Management (built-in) + Static Web Apps | Self-service portal | Standard |
| Microservices | Container Apps | Backend services, adapters | Consumption |
...
| Identity | Microsoft Entra ID + External ID | Authentication, RBAC | P1 |
| Messaging | Azure Service Bus | Webhooks, async processing | Standard |
...
|-------------------|---------------------|----------------|
| **Network Security (NS)** | NS-1, NS-2, NS-3, NS-5 | Front Door WAF, VNet, Private Link, NSGs |
| **Identity Management (IM)** | IM-1, IM-2, IM-3, IM-7 | Entra ID, Managed Identities, MFA, Conditional Access |
| **Privileged Access (PA)** | PA-1, PA-2, PA-7 | Entra ID PIM, JIT access, RBAC |
| **Data Protection (DP)** | DP-1, DP-2, DP-3, DP-4 | TLS 1.2+, AES-256, Key Vault CMK, PostgreSQL TDE |
| **Asset Management (AM)** | AM-1, AM-2 | Resource Graph, Azure Policy, Tags |
| **Logging & Threat Detection (LT)** | LT-1, LT-2, LT-3, LT-4 | Azure Monitor, Sentinel, Defender for Cloud |
...
| **Endpoint Security (ES)** | ES-1 | Managed platform (no endpoints to manage) |
| **Backup & Recovery (BR)** | BR-1, BR-2, BR-3 | PostgreSQL geo-backup, Blob immutable storage |
| **DevOps Security (DS)** | DS-1, DS-2, DS-6 | GitHub Advanced Security, SAST, dependency scanning |
| **Governance & Strategy (GS)** | GS-1, GS-2, GS-6 | Azure Policy, Management Groups, Security Benchmark |
...
module apim 'modules/apim.bicep' = {
  ...
  params: {
    ...
    publisherEmail: 'api-aggregator@digital.cabinet-office.gov.uk'
    publisherName: 'Government Digital Service'
  }
}
...
|----------|---------------|---------------|--------------|
| API Gateway | API Management | Premium, 1 unit | £2,500 |
| Container Platform | Container Apps | Consumption plan | £800 |
| Primary Database | PostgreSQL Flexible Server | GP, 4 vCores, HA | £600 |
| Caching | Azure Managed Redis | Standard C2 | £350 |
| Search | Azure AI Search | Standard S1 | £250 |
| Identity | Entra ID + External ID | P1 + per-auth | £200 |
...
| Year 2 | £5,640 | £67,680 | £137,400 | 3% savings via reserved instances |
| Year 3 | £6,390 | £76,680 | £214,080 | Growth to 15M calls/month |
| **Total** | | | **~£214,080** | |

### Cost Optimization Recommendations

1. **Reserved Instances**: Save up to 40% on PostgreSQL and Redis with 1-year reservations (~£340/month savings)
2. **APIM Standard v2**: Consider Standard v2 tier for Alpha/Beta phases (£250/month vs £2,500/month), upgrading to Premium for production when VNet integration is required
3. **Container Apps Consumption**: Pay-per-use pricing with scale-to-zero for batch workloads minimises waste
...
| 3. Separation | Per-department Key Vaults, managed identity isolation |
| 4. Governance | Azure Policy, Management Groups, RBAC |
...
| 11. External interface protection | Front Door WAF, NSGs, Private Link |
...
| APIM with AKS/Container Apps | https://learn.microsoft.com/azure/api-management/api-management-kubernetes |
...
5. **G-Cloud Procurement**: Initiate G-Cloud 14 call-off for Azure services
